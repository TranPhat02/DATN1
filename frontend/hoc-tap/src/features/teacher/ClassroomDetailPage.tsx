/**
 * ClassroomDetailPage — Teacher's classroom detail with tabs
 * Tabs: Tài liệu | Điểm môn học | Tạo TN | Làm TN | Điểm TN | Trò chuyện
 */
...
import TeacherDiemMonHocPage from './DiemMonHocPage';
import TeacherDiemTracNghiemPage from './DiemTracNghiemPage';
import TaiLieuTab from '../../shared/components/TaiLieuTab';
import QuizCreateTab from '../../shared/components/QuizCreateTab';
import QuizTakeTab from '../../shared/components/QuizTakeTab';
import ChamDiemTab from '../../shared/components/ChamDiemTab';
...
const TABS = [
  ...
  { key: 'diemmonhoc', label: 'Điểm môn', icon: <HiOutlineChartBar /> },
  { key: 'taotracnghiem', label: 'Tạo TN', icon: <HiOutlinePencilSquare /> },
  { key: 'lamtracnghiem', label: 'Danh sách TN', icon: <HiOutlineClipboardDocumentList /> },
  { key: 'chamdiem', label: 'Chấm điểm', icon: <HiOutlineDocumentText /> },
  { key: 'diemtracnghiem', label: 'Điểm TN', icon: <HiOutlineAcademicCap /> },
...
export default function ClassroomDetailPage() {
  ...
  const handleEditQuiz = (quiz: Quiz) => {
    setEditingQuiz(quiz);
    setActiveTab('taotracnghiem');
  };

  const renderTabContent = () => {
    switch (activeTab) {
      case 'tailieu':
        return <TaiLieuTab maLopMon={maLopMon!} canUpload canDelete />;
      case 'diemmonhoc':
        return <TeacherDiemMonHocPage maLopMon={maLopMon!} />;
      case 'taotracnghiem':
        return <QuizCreateTab maLopMon={maLopMon!} editingQuiz={editingQuiz} onCancelEdit={() => setEditingQuiz(null)} />;
      case 'lamtracnghiem':
        return <QuizTakeTab maLopMon={maLopMon!} onEditQuiz={handleEditQuiz} isTeacher={true} />;
      case 'chamdiem':
        return <ChamDiemTab maLopMon={maLopMon!} />;
      case 'diemtracnghiem':
        return <TeacherDiemTracNghiemPage maLopMon={maLopMon!} />;
      case 'trochuyen':
        // when opening chat, maybe we want to refresh count? 
        return <ChatTab maLopMon={maLopMon!} />;
      case 'thongbao':
        return <AnnouncementTab maLopMon={maLopMon!} isTeacher={true} />;
      case 'baitap':
        return <BaiTapTab maLopMon={maLopMon!} isTeacher={true} />;
      default:
        return null;
    }
  };
        ...
        {TABS.map((tab) => {
          ...
          <button
            ...
            onClick={() => {
              ...
              if (tab.key !== 'taotracnghiem') setEditingQuiz(null);
              if (user) {
                if (isChat) {
